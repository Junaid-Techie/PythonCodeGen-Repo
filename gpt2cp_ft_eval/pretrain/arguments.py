...
class TrainingArguments:
    ...
    """
    i : int  = field(
        default=0, metadata={"help": "Id of training dataset shard from 0 to 15"}
    )
    model_ckpt: Optional[str] = field(
        default="gpt2", metadata={"help": "Model name or path of model to be trained."}
    )
    ...
    dataset_name_train: Optional[str] = field(
        default=f"data/merged_data.jsonl", metadata={"help": "Name or path of training dataset."}
    )
    dataset_name_valid: Optional[str] = field(
        default="/home/students/jkatama/.cache/code-parrot-valid-stripped/valid_data.json", metadata={"help": "Name or path of validation dataset."}
    )
    ...
    learning_rate: Optional[float] = field(default=2e-4, metadata={"help": "Learning rate fo training."})  ## based on scaling laws in Deepmind paper 
    #(3e-4)*(47.5/40) ~ same 
    ## additionaly, small models can take big lrs
    ## but our batch is limited by vram of a16, we tend to use 2e-4
    lr_scheduler_type: Optional[str] = field(default="cosine", metadata={"help": "Learning rate."})
    num_warmup_steps: Optional[int] = field(
        default=750, metadata={"help": "Number of warmup steps in the learning rate schedule."} ## 1% of data
    )
    ...
    max_eval_steps: Optional[int] = field(
        default=512, metadata={"help": "Maximum number of evaluation steps. If -1 the full dataset is evaluated."}
    )
    ...
    resume_from_checkpoint: Optional[str] = field(
        default='step_36864', metadata={"help": "States path if the training should continue from a checkpoint folder."}
    )
    tokenized: Optional[bool] = field(default=False, metadata={"help": "If True the data is pretokenized."})


    # def __post_init__(self):
    #     self.dataset_name_train = f"data/shard_{self.i}.jsonl"
    # def update_shard_id(self):
    #     self.i += 1
    #     self.dataset_name_train = f"data/shard_{self.i}.jsonl"
...
class EvaluationArguments:
    ...
    model_ckpt: Optional[str] = field(
        default="gpt2", metadata={"help": "Model name or path of model to be evaluated."}
    )
...
class InitializationArguments:
    ...
    model_name: Optional[str] = field(default="gpt2", metadata={"help": "Name of the created model."})
    push_to_hub: Optional[bool] = field(default=False, metadata={"help": "Push saved tokenizer to the hub."})
